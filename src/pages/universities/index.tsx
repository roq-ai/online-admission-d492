...
type ColumnType = ColumnDef<UniversityInterface, unknown>;

interface UniversityListPageProps {
...
export function UniversityListPage(props: UniversityListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<UniversityInterface>>(
    () => `/universities?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: UniversityInterface) => {
    if (hasAccess('university', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/universities/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/universities/view/${record.id}`} passHref legacyBehavior>
                <Button
