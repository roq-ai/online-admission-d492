...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'New Student Application.create': { roles: ['administrator'], key: 'application_id', tenantPath: [], userPath: [] },
...
const tenantRoles: string[] = ['administrator', 'admissions-officer', 'student'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.university.findFirst({
